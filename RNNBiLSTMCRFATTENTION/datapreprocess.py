...
def vocab_build(config):
    ...
    for i in range(feature_num):
        ...
        with open(config['data_params']['feature_params'][i]['voc_path'], 'wb') as fw:
...
def embedding_build(config):
    ...
    for i in range(feature_num):
        ...
        if pre_train:
            embed,fea2id=load_emb(config['model_params']['embed_params'][i]['path'])
        else:
            fea2id=read_dictionary(config['data_params']['feature_params'][i]['voc_path'])

            embed_dim=config['model_params']['embed_params'][i]['dimension']
            embed=random_embedding(fea2id,embed_dim)
        with open(config['data_params']['feature_params'][i]['embed_path'], 'wb') as fw:
...
def embedding_load(config):
    ...
    for i in range(feature_num):
        ...
        embed=read_dictionary(config['data_params']['feature_params'][i]['embed_path'])
...
#3.数据初始化
def init_data(feature_num,data_path,fea2id_list,label2id):
    data=[]

    #print(len(fea2id_list))
    fr=open(data_path, encoding='utf-8')
