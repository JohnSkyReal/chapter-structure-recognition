...
# 2.加载词向量
print('加载embedding')
fea2id_list,feature_embedding_list=embedding_load(config)
...
output_path = os.path.join('.', config['model_params']['path_save'])
# 6.数据初始化

print('数据初始化')
data=init_data(feature_num,config['data_params']['path_test'],fea2id_list,label2id)
# 7.模型初始化
print('创建模型')
